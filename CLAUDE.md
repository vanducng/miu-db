...
`miu-db` (PyPI: `miu-db`) — a Textual-based terminal UI for 25+ SQL databases. Entry point: `miu_db.cli:main`. Requires Python 3.10+.
...
### Run locally
```bash
miu-db                          # launch TUI, pick a saved connection
miu-db --mock=sqlite-demo       # launch against mock adapter (no real DB)
miu-db -c "Name"                # open specific saved connection
miu-db query -c "Name" -q "SELECT ..."   # non-interactive CLI mode
miu-db postgresql://user:pass@host/db    # URL-based ad-hoc connect
```
...
mypy miu_db            # strict: disallow_untyped_defs, warn_return_any, strict_optional
python -m build        # sdist + wheel via hatchling (version from VCS tag)
nix build .#miu-db     # flake build (mirrored in CI)
```

## Architecture

### Layout
Source lives in `miu_db/` (not `src/miu_db/`). Three top-level layers:

- `miu_db/cli.py` — argparse entry: parses URLs/subcommands, builds runtime, dispatches to TUI or CLI subcommands (`connections`, `connect`, `query`).
- `miu_db/core/` — app-wide modal input machinery shared by every screen: `vim.py` (motion engine), `keymap.py`, `key_router.py`, `binding_contexts.py`, `input_context.py`, `leader_commands.py`, `state_base.py`, `connection_manager.py`.
- `miu_db/domains/<domain>/` — feature slices. Each domain follows the same sub-layout where applicable:
...
- `miu_db/shared/` — cross-domain infra:
...
- `query/` — query editor + execution. `completion/` is a per-statement-type autocomplete engine (`create_table.py`, `insert.py`, `update.py`, …; `completion.py` is C901-exempt). `app/query_runner.py` + `cancellable.py` + `multi_statement.py` + `transaction.py` drive execution; `editing/` integrates with `miu_db/core/vim.py`.
...
### Provider pattern
Adding a new database means: drop a package under `miu_db/domains/connections/providers/<db>/` implementing the adapter protocol, register it in `catalog.py`/`registry.py`, declare its driver in `driver.py` (for the auto-install wizard), add an optional-dependency extra in `pyproject.toml`, and add a `tests/test_<db>.py` integration file (plus a compose service in `infra/docker/docker-compose.test.yml` when possible).

### Runtime composition
`miu_db/cli.py` → `RuntimeConfig` (flags, mock config, startup profiler) → `build_app_services()` in `miu_db/shared/app/services.py` wires stores, process runners, provider factories, system probe, and driver resolver. Tests substitute any of these via the protocols in `miu_db/shared/core/protocols.py` and the fakes in `miu_db/shared/core/system_probe_fake.py` / `FixedResult*Runner`.
