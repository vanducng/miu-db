{
  description = "miu-db — a fast, modal TUI for SQL databases. Vim keys, 25+ adapters, zero config.";
  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        resolveExtras = names:
          lib.concatLists (map (n:
            nixpkgsExtras.${n} or (throw
              "Unknown miu-db extra '${n}'. Available: ${
                lib.concatStringsSep ", " (builtins.attrNames nixpkgsExtras)
              }")
          ) names);

        # Build miu-db, optionally with a set of driver extras from nixpkgs.
        # Example:  makeMiuDb { extras = [ "postgres" "ssh" ]; }
        makeMiuDb = { extras ? [] }:
          pyPkgs.buildPythonApplication {
            ...
            pythonImportsCheck = [ "miu_db" ];

            meta = with lib; {
              description = "A fast, modal TUI for SQL databases. Vim keys, 25+ adapters, zero config.";
        ...
        miu-db = makeMiuDb { extras = builtins.attrNames nixpkgsExtras; };
      in {
        packages = {
          miu-db = miu-db;
          default = miu-db;
        ...
        lib = { makeMiuDb = makeMiuDb; };

        apps.default = {
          type = "app";
          program = "${miu-db}/bin/miu-db";
        };
