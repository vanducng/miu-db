...
Thank you for considering a contribution to miu-db! This guide walks you through setting up your environment, running the test suite, and understanding the CI expectations.

## Development Setup

1. Clone the repository:
   ```bash
   git clone https://github.com/vanducng/miu-db.git
   cd miu-db
   ```
...
| `POSTGRES_DATABASE` | `test_miu_db` | PostgreSQL database |
...
| `MYSQL_DATABASE` | `test_miu_db` | MySQL database |
...
| `COCKROACHDB_DATABASE` | `test_miu_db` | CockroachDB database |
...
| `FIREBIRD_DATABASE` | `/var/lib/firebird/data/test_miu_db.fdb` | Firebird database path or alias |
...
If an idea or feature does *not* achieve any of the 'CBQV' elements adhering to all of the 'EAFF' requirements. It does not belong to miu-db.

**[E]asy:**
miu-db should not require any external documentation at all. It must prioritize intuitiveness above all.

**[A]esthetically pleasing:**
miu-db should not render one unnecessary pixel. It should prioritize beauty above anything. Minimalism over bloat.

**Fun:**
miu-db aims make fulfilling its core purpose be an enjoyable experience for the user, even a source of pleasure.

**Fast:**
miu-db aims to fulfill its core purpose for the user, with intention to giving the user the results they want with as few actions as possible.

Essentially, miu-db aims to do CRUD on SQL really well.

This implies this tool is more suited for developer's daily use than an database administrator.
Every feature in miu-db should have a target audience in which they will use it every time they use miu-db.
If nobody is going to a feature every day. It does not belong to miu-db.
E.g.

1) advanced query performance debugging -> rarely used -> does not belong in miu-db
2) edit cell key-binding -> a audience who will use this every day -> belongs to miu-db

**On complexity:**
Minimalism: miu-db aims to abstract away as much complexity as possible from the user, while giving them enough control to achieve CBQV. miu-db should never do anything under the hood that the user might have interest in understanding. Universal state problems deem for magical fixes. Conditional state problems, explicit user awareness. User should never ask "wait, how did it know?" "why is this here?" "why did it work then, but not now?"
...
To make miu-db as fast' as possible, miu-db has a large focus on keybindings.
To make miu-db as 'easy' as possible, all necessary keybindings to do 'CEQR' well, must be visible at all times.
To make miu-db as 'aesthetically pleasing' any keybinding not strictly necessary to perform 'CEQR' in a 'easy' and 'fast' way will be hidden behind help <?> or command menu <space>
...
**Ideal:**
It should be easy to use for someone who just started using miu-db.
miu-db should provide fun and a feeling of mastery and satisfaction for those who want to achieve it, by becoming a sql-manipulating wizard with creative keybinding combos.
...
User-facing keymap overrides live in `settings.json` under `keymap.overrides`. The allowlist is `REBINDABLE_ACTIONS` in `miu_db/core/keymap.py`. Expanding it is a deliberate UX decision — don't widen without a brainstorm, since most keys (motions, chords, leader menus) have structural assumptions.
