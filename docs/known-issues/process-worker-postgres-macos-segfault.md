...
Happy-path behaviour (`MIU_DB_PROCESS_WORKER=0`, SQLite, DuckDB, BigQuery, …)
is unaffected.

## Root cause

`miu-db` runs schema listings and queries in a child process via
...
1. **Provider-level opt-out on macOS.**
   ...
   - Files: `miu_db/domains/connections/providers/postgresql/adapter.py`
...
2. **Migrate to `psycopg` (v3).**
   ...
   - Files: `miu_db/domains/connections/providers/postgresql/*`,
