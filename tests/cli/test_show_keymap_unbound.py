"""Test that `miu-db config show-keymap` lists unbound rebindable actions."""
...
from miu_db.domains.shell.cli.config import run_show_keymap


def _capture_show_keymap(settings: dict | None = None) -> str:
    buf = StringIO()
    with (
        patch("miu_db.core.configurable_keymap.load_overrides_from_settings", return_value=(settings or {}).get("overrides", {})),
