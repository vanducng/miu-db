...
_TEST_CONFIG_DIR = Path(tempfile.mkdtemp(prefix="miu-db-test-config-"))
os.environ.setdefault("MIU_DB_CONFIG_DIR", str(_TEST_CONFIG_DIR))
...
def run_cli(*args: str, check: bool = True) -> subprocess.CompletedProcess:
    """Run miu-db CLI command and return result."""
    cmd = [sys.executable, "-m", "miu_db.cli"] + list(args)
