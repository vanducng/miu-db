...
from miu_db.domains.connections.domain.config import AuthType, ConnectionConfig, DatabaseType
from miu_db.domains.connections.providers.catalog import get_provider_schema, get_supported_db_types
...
def _get_schema_value_flags() -> set[str]:
    from miu_db.domains.connections.providers.catalog import iter_provider_schemas
...
def _extract_connection_url(argv: list[str]) -> tuple[str | None, list[str]]:
    ...
    """
    from miu_db.domains.connections.app.url_parser import is_connection_url
...
def _resolve_startup_log_path(argv: list[str]) -> Path | None:
    ...
    if profile_enabled and log_path is None:
        log_path = Path(".miu-db") / "startup.txt"
    return log_path
...
def _resolve_startup_import_settings(argv: list[str]) -> tuple[Path | None, float]:
    ...
    if enabled and log_path is None:
        log_path = Path(".miu-db") / "startup-imports.txt"
...
def _exec_restart(argv: list[str]) -> None:
    ...
    # execv doesn't search PATH; use execvp for bare commands (e.g. "miu-db").
    if has_sep:
        os.execv(exe, argv)
    else:
        os.execvp(exe, argv)
...
def _build_runtime(args: argparse.Namespace, startup_mark: float) -> RuntimeConfig:
    ...
    if import_profile_enabled and startup_import_log_path is None:
        startup_import_log_path = Path(".miu-db") / "startup-imports.txt"
    ...
    if profile_startup and startup_log_path is None:
        startup_log_path = Path(".miu-db") / "startup.txt"
...
def main() -> int:
    ...
    parser.add_argument(
        "--settings",
        metavar="PATH",
        help="Path to settings JSON file (overrides the one in the miu-db config directory)",
    )
    parser.add_argument(
        "--theme",
        metavar="NAME",
    ...
    # Config management
    config_parser = subparsers.add_parser("config", help="Manage miu-db configuration")
    ...
    if args.command is None:
        ...
        if args.mock:
            from miu_db.domains.connections.app.mocks import get_mock_profile, list_mock_profiles
        ...
        if getattr(app, "_restart_requested", False):
            ...
            try:
                ...
                print(f"Failed to restart miu-db: {exc}", file=sys.stderr)
                return 1
        return 0

    from miu_db.domains.connections.cli.commands import (
    ...
    if args.command == "docker":
        from miu_db.domains.connections.cli.commands import cmd_docker_list
...
def _apply_keymap_overrides() -> None:
    ...
    from miu_db.core.configurable_keymap import (
        ConfigurableKeymapProvider,
        load_overrides_from_settings,
    )
    from miu_db.core.keymap import set_keymap
...
def _dispatch_config(args: argparse.Namespace, config_parser: argparse.ArgumentParser) -> int:
    from miu_db.domains.shell.cli.config import run_edit, run_show_keymap
