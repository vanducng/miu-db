...
from miu_db.domains.shell.app.idle_scheduler import init_idle_scheduler
from miu_db.shared.app.startup_profiler import write_line
...
def run_on_mount(app: AppProtocol) -> None:
    ...
    if "query_alert_mode" in settings:
        from miu_db.domains.query.app.alerts import parse_alert_mode
...
def _warn_on_missing_actions(app: AppProtocol, is_headless: bool) -> None:
    from miu_db.core.action_validation import validate_actions
    ...
    if is_headless:
        print(f"[miu-db] {message}", file=sys.stderr)
        return
    try:
...
def _warn_on_keyring_error(app: AppProtocol, is_headless: bool) -> None:
    from miu_db.domains.connections.app.credentials import get_keyring_probe_error
...
def log_startup_timing(app: AppProtocol) -> None:
    ...
    _emit_startup_line(app, f"[miu-db] startup {' '.join(parts)}")
    ...
    def after_refresh() -> None:
        ...
        _emit_startup_line(app, f"[miu-db] startup {' '.join(refresh_parts)}")
...
def maybe_auto_connect_pending(app: AppProtocol) -> bool:
    ...
    """
    from miu_db.shared.core.debug_events import emit_debug_event

    from miu_db.domains.connections.ui.restart_cache import (
        clear_restart_cache,
