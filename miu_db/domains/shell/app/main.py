"""Main Textual application for miu-db."""
...
from miu_db.domains.query.ui.mixins.autocomplete import AutocompleteMixin
from miu_db.domains.query.ui.mixins.query import QueryMixin
from miu_db.domains.results.ui.mixins.results import ResultsMixin
from miu_db.domains.results.ui.mixins.results_filter import ResultsFilterMixin
from miu_db.domains.shell.app.commands import dispatch_command
from miu_db.domains.shell.app.idle_scheduler import IdleScheduler
from miu_db.domains.shell.app.omarchy import DEFAULT_THEME
from miu_db.domains.shell.app.startup_flow import run_on_mount
from miu_db.domains.shell.app.theme_manager import ThemeManager
from miu_db.domains.shell.state import UIStateMachine
from miu_db.domains.shell.state.layout_state import LayoutState
from miu_db.domains.shell.ui.mixins.ui_navigation import UINavigationMixin
from miu_db.shared.app import AppServices, RuntimeConfig, build_app_services
from miu_db.shared.core.debug_events import (
    DebugEvent,
    DebugEventBus,
    format_debug_data,
...
from miu_db.shared.ui.protocols import AppProtocol, UINavigationMixinHost
from miu_db.shared.ui.widgets import (
...
from miu_db.shared.ui.widgets_stacked_results import StackedResultsContainer
...
class SSMSTUI(
    ...
    def _get_input_context(self) -> InputContext:
        ...
        if self._value_view_active:
            try:
                from miu_db.shared.ui.widgets import InlineValueView
    ...
    def _set_debug_events_enabled(self, enabled: bool) -> None:
        ...
        if enabled:
            try:
                from miu_db.core.keymap import emit_keybinding_snapshot
    ...
    def _show_command_list(self) -> None:
        ...
        rows = [
            ...
            ("General", ":q, :quit, :exit", "Quit miu-db", ""),
            ...
            (
                ...
                "Use 'plaintext' to store passwords in the miu-db config directory (protected folder), 'keyring' to use system keyring.",
            ),
    ...
    def _show_version_info(self) -> None:
        try:
            from miu_db import __version__ as pkg_version
        except Exception:
            pkg_version = "unknown"
    ...
    def autocomplete_dropdown(self) -> Any:
        from miu_db.shared.ui.widgets import AutocompleteDropdown
            ...
            with Horizontal(id="content"):
                ...
                with Vertical(id="main-panel"):
                    ...
                    with Container(id="results-area"):
                        ...
                        yield Lazy(MiuDataTable(id="results-table", zebra_stripes=True, show_header=False))
    ...
    def get_theme_variable_defaults(self) -> dict[str, str]:
        from miu_db.domains.shell.app.themes import DEFAULT_MODE_COLORS
