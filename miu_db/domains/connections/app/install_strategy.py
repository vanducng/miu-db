"""Detection for how miu-db should suggest/install optional Python drivers.
...
def get_install_options(
    *,
    ...
    all_options = {
        ...
        "pipx": InstallOption("pipx", f"pipx inject miu-db {shell_target('pipx')}"),
        "uv": InstallOption("uv", f"uv pip install {shell_target('uv')}"),
        "uv-tool": InstallOption(
            "uv-tool",
            f"uv tool install --reinstall --with {shell_target('uv-tool')} miu-db",
        ),
        "uvx": InstallOption(
            "uvx",
            f"uvx --from miu-db --with {shell_target('uvx')} miu-db",
        ),
...
def detect_strategy(
    *,
    ...
    if install_method == "pipx":
        ...
        cmd = ["pipx", "inject", "miu-db", install_target]
        return InstallStrategy(
            kind="pipx",
            can_auto_install=True,
            manual_instructions="pipx inject miu-db " + _format_shell_target(install_target),
