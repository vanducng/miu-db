"""CLI command handlers for miu-db."""
...
def _maybe_prompt_plaintext_credentials(services: AppServices) -> bool:
    ...
    answer = input("Keyring isn't available. Save passwords as plaintext in the miu-db config directory? [y/N]: ").strip().lower()
...
def cmd_connection_create(args: Any, *, services: AppServices | None = None) -> int:
    ...
    from miu_db.domains.connections.app.url_parser import is_connection_url, parse_connection_url
    ...
    if not getattr(args, "provider", None):
        ...
        print("Examples:")
        print("  miu-db connections add postgresql --name MyDB --server localhost ...")
        print("  miu-db connections add --url postgresql://user:pass@host/db --name MyDB")
        return 1
