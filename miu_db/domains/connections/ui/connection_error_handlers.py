...
from miu_db.shared.ui.protocols import ConnectionsProtocol, TextualAppProtocol
...
class AzureFirewallHandler:
    ...
    def can_handle(self, error: Exception) -> bool:
        from miu_db.domains.connections.discovery.cloud.azure.firewall import is_firewall_error

        return is_firewall_error(str(error))

    def handle(self, app: ConnectionErrorApp, error: Exception, config: ConnectionConfig) -> None:
        from miu_db.domains.connections.discovery.cloud.azure.firewall import (
