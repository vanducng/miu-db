...
from miu_db.domains.connections.app.connection_flow import ConnectionFlow, ConnectionPrompter
...
from miu_db.shared.ui.protocols import ConnectionMixinHost
...
class ConnectionMixin:
        ...
        def on_success(session: ConnectionSession) -> None:
            ...
            if not reconnected:
                ...
                if getattr(self, "_pending_telescope_query", None) or getattr(self, "_defer_schema_load", False):
                    ...
                    try:
                        from miu_db.domains.shell.app.idle_scheduler import (
                            Priority,
                            get_idle_scheduler,
                        )
                    except Exception:
    ...
    def _set_connection_screen_footer(self: ConnectionMixinHost) -> None:
        from miu_db.shared.ui.widgets import ContextFooter
            ...
            if needs_password_persist and not is_keyring_usable():
                ...
                def on_confirm(confirmed: bool | None) -> None:
                    ...
                    if confirmed is True:
                        ...
                        self.services.connection_store.set_credentials_service(self.services.credentials_service)
                        do_save(config, original_name)
                        self.notify("Saved passwords as plaintext in the miu-db config directory (0600)", severity="warning")
                        return
                ...
                self.push_screen(
                    ConfirmScreen(
                        ...
                        "Save passwords as plaintext in the miu-db config directory (protected directory)?",
                        yes_label="Yes",
                        no_label="No",
                    ),
                    on_confirm,
    ...
    def action_move_connection_to_folder(self: ConnectionMixinHost) -> None:
        from miu_db.domains.connections.app.credentials import CredentialsPersistError
        from miu_db.domains.connections.domain.config import normalize_folder_path
        from miu_db.domains.connections.ui.screens import FolderInputScreen
        from miu_db.shared.ui.screens.error import ErrorScreen
