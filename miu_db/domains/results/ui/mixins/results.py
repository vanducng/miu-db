...
from miu_db.shared.ui.protocols import ResultsMixinHost
...
class ResultsMixin:
    ...
    def _copy_text(self: ResultsMixinHost, text: str) -> bool:
        ...
        from miu_db.shared.ui.clipboard import copy_to_system_clipboard
    ...
    def _get_active_results_context(
        self: ResultsMixinHost,
    ) -> tuple[MiuDataTable | None, list[str], list[tuple], bool]:
        """Get the active results table and data, handling stacked mode."""
        if self.results_area.has_class("stacked-mode"):
            try:
                from miu_db.shared.ui.widgets_stacked_results import ResultSection, StackedResultsContainer

                container = self.query_one("#stacked-results", StackedResultsContainer)
                focused_table = next(
                    (table for table in container.query(MiuDataTable) if table.has_focus),
                    None,
                )
                section = None
                table = None
                ...
                if focused_table is not None:
                    ...
                    try:
                        table = section.query_one(MiuDataTable)
                    except Exception:
                        table = None
    ...
    def _find_results_section(self: ResultsMixinHost, widget: Any) -> Any | None:
        ...
        from miu_db.shared.ui.widgets_stacked_results import ResultSection
    ...
    def _flash_table_yank(self: ResultsMixinHost, table: MiuDataTable, scope: str) -> None:
    ...
    def action_copy_value_view(self: ResultsMixinHost) -> None:
        ...
        """
        from miu_db.shared.ui.widgets import InlineValueView, flash_widget
    ...
    def _hide_cell_tooltip(self: ResultsMixinHost, table: MiuDataTable) -> None:
    ...
    def action_ry_row_json(self: ResultsMixinHost) -> None:
        ...
        triggers a warning.
        """
        from miu_db.domains.results.formatters import format_row_json
    ...
    def _save_export_file(self: ResultsMixinHost, filename: str, fmt: str) -> None:
        ...
        from miu_db.domains.results.formatters import format_csv, format_json
