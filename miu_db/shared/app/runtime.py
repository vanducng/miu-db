...
class RuntimeConfig:
    ...
    def from_env(cls) -> RuntimeConfig:
        ...
        settings_path = (os.environ.get("MIU_DB_SETTINGS_PATH") or "").strip() or None
        startup_log_path = (os.environ.get("MIU_DB_PROFILE_STARTUP_FILE") or "").strip() or None
        startup_exit = os.environ.get("MIU_DB_PROFILE_STARTUP_EXIT") == "1"
        import_log_path = (os.environ.get("MIU_DB_PROFILE_STARTUP_IMPORTS_FILE") or "").strip() or None
        import_enabled = os.environ.get("MIU_DB_PROFILE_STARTUP_IMPORTS") == "1" or bool(import_log_path)
        import_min_raw = (os.environ.get("MIU_DB_PROFILE_STARTUP_IMPORTS_MIN_MS") or "").strip()
        import_min_ms = _parse_float(import_min_raw) if import_min_raw else 1.0
        profile_startup = (
            os.environ.get("MIU_DB_PROFILE_STARTUP") == "1"
        ...
        max_rows = _parse_int(os.environ.get("MIU_DB_MAX_ROWS"))
        worker_env = os.environ.get("MIU_DB_PROCESS_WORKER")
        if worker_env is None or not worker_env.strip():
            process_worker = True
        else:
            process_worker = worker_env.strip().lower() in {"1", "true", "yes", "on"}
        warm_env = os.environ.get("MIU_DB_PROCESS_WORKER_WARM_ON_IDLE")
        process_worker_warm_on_idle = _parse_bool(warm_env, True)
        shutdown_env = os.environ.get("MIU_DB_PROCESS_WORKER_AUTO_SHUTDOWN_S")
        process_worker_auto_shutdown_s = _parse_float(shutdown_env)
        stall_env = os.environ.get("MIU_DB_UI_STALL_WATCHDOG_MS")
        ui_stall_watchdog_ms = _parse_float(stall_env)
        missing_drivers = os.environ.get("MIU_DB_MOCK_MISSING_DRIVERS") or ""
        missing_driver_set = {item.strip() for item in missing_drivers.split(",") if item.strip()}

        mock_config = MockConfig(
            missing_drivers=missing_driver_set,
            install_result=(os.environ.get("MIU_DB_MOCK_INSTALL_RESULT") or "").strip().lower() or None,
            pipx_mode=(os.environ.get("MIU_DB_MOCK_PIPX") or "").strip().lower() or None,
            query_delay=_parse_float(os.environ.get("MIU_DB_MOCK_QUERY_DELAY")),
            demo_rows=_parse_int(os.environ.get("MIU_DB_DEMO_ROWS")) or 0,
            demo_long_text=os.environ.get("MIU_DB_DEMO_LONG_TEXT") == "1",
            cloud=os.environ.get("MIU_DB_MOCK_CLOUD") == "1",
            driver_error=os.environ.get("MIU_DB_MOCK_DRIVER_ERROR") == "1",
        )

        return cls(
            settings_path=Path(settings_path).expanduser() if settings_path else None,
            max_rows=max_rows,
            debug_mode=os.environ.get("MIU_DB_DEBUG") == "1",
            debug_idle_scheduler=os.environ.get("MIU_DB_DEBUG_IDLE_SCHEDULER") == "1",
            profile_startup=profile_startup,
            startup_mark=_parse_startup_mark(os.environ.get("MIU_DB_STARTUP_MARK")),
